...
/** TDAmeritrade's API server - Worked on by Henry Shi **/
/*
import Vue from 'vue'
...
export default new Vue({
  ...
  actions: {
            ...
            dispatch("refreshTdAccessToken")
              .then(result => {
                if (result) {
                  ...
                  reject("Access token and refresh token do not exist. Please re-try login.")
                }
              })
          }
        })
    ...
    getTdAccessTokenFromCode({commit, state}, code) {
      ...
      axios.post(
        "https://api.tdameritrade.com/v1/oauth2/token", qs.stringify(body))
        .then(response => {
    ...
    refreshTdAccessToken({commit, getters, state}) {
      ...
      const body = { "grant_type": "refresh_token", "refresh_token": refreshToken,
                    "client_id": process.env.VUE_APP_TD_CONSUMER_KEY, }
      return axios.post("https://api.tdameritrade.com/v1/oauth2/token", qs.stringify(body))
        .then(response => {
          console.log(response);
          commit('setTdAccessToken', response.data.access_token, response.data.expires_in);
